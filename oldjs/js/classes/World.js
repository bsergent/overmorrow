define(["require", "exports"], function (require, exports) {
    ...
    define(['interface', 'util', 'lib/lodash', 'classes/Ship', 'lib/perlin', 'moment', 'jquery', 'classes/TiledMap', 'classes/UIPanel'], function (ui, util, _, Ship, Perlin, moment, $, TiledMap, UIPanel) {
        return function (width, height) {
            ...
            this.draw = function () {
                var startTime = moment();
                if (player)
                    ui.centerViewPort(player.position.x, player.position.y, this);
                ui.drawRect(0, 0, ui.getWidth(), ui.getHeight(), '#85afcc');
                ui.setOffset(player.velocity * 150 * Math.cos(player.position.heading * Math.PI / 180), player.velocity * 150 * Math.sin(player.position.heading * Math.PI / 180));
                // Draw world
                tilemap.drawBG(ui);
                //ui.setAA(true);
                for (var e of this.entities)
                ...
                ui.drawRect(ui.getWidth() - MINIMAP_WIDTH, ui.getHeight() - MINIMAP_WIDTH / height * width, MINIMAP_WIDTH, MINIMAP_WIDTH / height * width, 'rgba(112,112,112,0.5)');
                for (var y = Math.floor(MINIMAP_RESOLUTION / 2); y < height; y += MINIMAP_RESOLUTION) {
                    for (var x = Math.floor(MINIMAP_RESOLUTION / 2); x < width; x += MINIMAP_RESOLUTION) {
                        ...
                        ui.drawRect(ui.getWidth() - MINIMAP_WIDTH + ((x - MINIMAP_RESOLUTION / 2) * MINIMAP_WIDTH / width), ui.getHeight() - (MINIMAP_WIDTH / height * width) + ((y - MINIMAP_RESOLUTION / 2) * (MINIMAP_WIDTH / height * width) / height), MINIMAP_WIDTH / width * MINIMAP_RESOLUTION, (MINIMAP_WIDTH / height * width) / height * MINIMAP_RESOLUTION, color);
                    }
                }
                ui.drawRect(ui.getWidth() - MINIMAP_WIDTH + (player.position.x * MINIMAP_WIDTH / width) - MINIMAP_WIDTH / 50, ui.getHeight() - (MINIMAP_WIDTH / height * width) + (player.position.y * (MINIMAP_WIDTH / height * width) / height) - ((MINIMAP_WIDTH / height * width) / 50), MINIMAP_WIDTH / 50, (MINIMAP_WIDTH / height * width) / 50, 'red');
