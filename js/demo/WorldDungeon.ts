...
export default class Dungeon extends WorldSandbox {
  ...
  private _gen1State: GenState1 = GenState1.RANDOM_ROOMS;
  private _gen1RoomCount: number = 10;
  private _gen1RoomWidthMin: number = 3;
  private _gen1RoomWidthMax: number = 10;
  ...
  private _tick1(delta: number): void {
    switch (this._gen1State) {
      case GenState1.RANDOM_ROOMS:
        // Create room of random sizes
        let w: number, h: number;
        w = this._rand.intBetween(this._gen1RoomWidthMin, this._gen1RoomWidthMax);
        ...
        this._gen1State = GenState1.SPREAD_ROOMS;
        break;

      case GenState1.SPREAD_ROOMS:
        ...
        if (intersectingRoom === -1) {
          ...
          if (this._gen1CurrentRoomIndex >= this._gen1RoomCount)
            this._gen1State = GenState1.MOVE_ONSCREEN;
          else
            this._gen1State = GenState1.RANDOM_ROOMS;
          break;
        }
        ...
        if (this._gen1RoomOffset.magnitude === 0) {
          this._gen1RoomOffset = new Vector(
            room.center.x - this._rooms[intersectingRoom].center.x,
            room.center.y - this._rooms[intersectingRoom].center.y
          );
      ...
      case GenState1.MOVE_ONSCREEN:
        ...
        this._gen1State = GenState1.MIN_TREE;
        break;

      case GenState1.MIN_TREE:
        // TODO Carve passages in a maze-like manner based off the minimum spanning tree
        this._gen1State = GenState1.COMPLETE;
        break;
    }
  }
  ...
  private _gen2State: GenState2 = GenState2.INITIAL;
  ...
  private _tick2(delta: number): void {
    switch (this._gen2State) {
      case GenState2.INITIAL:
        ...
        this._gen2State = GenState2.PERFECT;
        ...
        console.log(`Starting at ${this._gen2Active[0].x}x, ${this._gen2Active[0].y}y.`);
        // Carve maze
      ...
      case GenState2.PERFECT:
        ...
        } else this._gen2State = GenState2.SPARSEN;
        break;

      case GenState2.SPARSEN:
        ...
        if (this._gen2DirtFilled >= this._gen2DirtToFill) this._gen2State = GenState2.DOORS;
        break;
      
      case GenState2.DOORS:
        for (let r = 0; r < this.height; r++) {
          for (let c = 0; c < this.width; c++) {
        ...
        this._gen2State = GenState2.ROOMS;
        break;
      
      case GenState2.ROOMS:
        ...
        if (this._rooms.length >= this._gen2RoomCount) this._gen2State = GenState2.CONNECT_ROOMS;
        break;

      case GenState2.CONNECT_ROOMS:
      // Carve rooms and corridors into tilemap
      for (let r = 0; r < this._rooms.length-1; r++) {
        let r1: Rectangle = this._rooms[r];
        let r2: Rectangle = this._rooms[r+1];
        ...
        this._gen2State = GenState2.COMPLETE;
        break;
    }
  }
}
