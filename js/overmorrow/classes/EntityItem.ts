...
export default class EntityItem extends Entity {
  ...
  private _pulseFading: boolean = false;
  private _pulseOpacity: number = 1.0; //Math.random() * 0.3 + 0.7;
  private _pulseSpeed: number = 0.025;
  public item: Item;

  constructor(x: number, y: number, item: Item, despawnSeconds: number = -1) {
    super(x + 0.25, y + 0.25, 0.5, 0.5, 'item', 0, 0);
  ...
  public draw(ui: WorldRenderer): void {
    if (this._pulseFading)
      this._pulseOpacity -= this._pulseSpeed;
    else
      this._pulseOpacity += this._pulseSpeed;
    if (this._pulseOpacity < 0.7)
      this._pulseFading = false;
    if (this._pulseOpacity > 1.0)
      this._pulseFading = true;
    if (DEBUG)
      ui.drawRectWire(this.clone().offset(0, (1 - this._pulseOpacity - 0.15) / 4), this._pulseSpeed < 0.05 ? Color.green : Color.red);
    ui.drawImage(
      this.clone().offset(0, (1 - this._pulseOpacity - 0.15) / 4),
      this.item.image,
      0,
